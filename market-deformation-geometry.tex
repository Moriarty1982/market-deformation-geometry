...
measured 120-day value by a factor of four. And the escape route
is named rather than hidden: if the true single-window estimation
floor were $\geq0.070$ --- $1.4$ times the measured split-half
floor, which is itself biased downward by the shared volatility
path --- the entire curve would be compatible with ordinary
diffusion; the registered next step is a volatility-clustering
(GARCH-class) null. Until that null is run, the honest summary of
